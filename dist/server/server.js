...
eval("var cart = __webpack_require__(/*! ./cart */ \"./src/server/cart.js\");\n\nvar fs = __webpack_require__(/*! fs */ \"fs\");\n\nvar actions = {\n  add: cart.add,\n  check: cart.check,\n  remove: cart.remove,\n  clear: cart.clear,\n  changeName: cart.changeName //const logger = require('./logger');\n\n};\n\nvar handlerSessions = function handlerSessions(req, res, action, file, sessionID) {\n  var userID = res.locals.user.id;\n  fs.readFile(file, 'utf-8', function (err, data) {\n    if (err) {\n      res.sendStatus(404, JSON.stringify({\n        result: 0,\n        text: err\n      }));\n    } else {\n      var _actions$action = actions[action](JSON.parse(data), req, userID, sessionID),\n          newCart = _actions$action.newCart,\n          name = _actions$action.name;\n\n      fs.writeFile(file, newCart, function (err) {\n        if (err) {\n          console.log('Ошибка записи файла ' + file);\n        } else {//\t\t\t\t\tlogger(name, action);\n        }\n      });\n    }\n  });\n};\n\nmodule.exports = handlerSessions;\n\n//# sourceURL=webpack:///./src/server/handlerSessions.js?");
